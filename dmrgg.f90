...
contains
 subroutine dtt_dmrgg(arg, rnk, pivpar, par)
  ...
  integer,pointer :: r(:),n(:)
  integer :: info,l,m,i,j,k,p,q,ii,jj,kk,qq,s,t,pp,it,dir,lft,rgt,pos,ijkq,ij,jk,kq,nlot,ilot,pivoting
  double precision :: aval,pivot
  double precision,allocatable :: a(:,:,:,:),b(:)
  double precision,allocatable :: brow(:,:,:),bcol(:,:,:),bcol1(:,:),brow1(:,:),acol1(:,:),arow1(:,:)
  double precision,allocatable :: mat(:,:),inv(:,:)
  integer,allocatable :: vip(:,:,:,:), lot(:,:)
  ...
  ! init
  lft=1;rgt=2
  allocate(vip(l-1:m,2,rnk,2),mat(rnk**2,l-1:m),inv(rnk**2,l-1:m),stat=info)
  ...
  do it=1,rnk-1
    ...
    do pp=l,m-1
     ...
     case(1,2,3) ! partial pivoting on O(r^a n) random elements, a=pivoting
      ...
      if(info.ne.0)then;write(*,*)subnam,': cannot allocate bcol1/brow1';stop;endif
     ...
     if(dabs(pivot).gt.1.e-14)then
      ...
      allocate(bcol(r(p-1),n(p),r(p)+1),brow(r(p)+1,n(p+1),r(p+1)), &
               bcol1(r(p-1),n(p)),brow1(n(p+1),r(p+1)), &
               stat=info)
      ...
      arg%r(p)=arg%r(p)+1
      deallocate(bcol,brow,bcol1,brow1)
     end if
  ...
  ! prepare the result 
  do p=l,m-1 
   call d2_lua(r(p-1)*n(p),n(p+1)*r(p+1),r(p),inv(1,p),arg%u(p)%p,arg%u(p+1)%p,u(p)%p,v(p+1)%p)
   call dcopy(r(p-1)*n(p)*r(p),u(p)%p,1,arg%u(p)%p,1)
   call dcopy(r(p)*n(p+1)*r(p+1),v(p+1)%p,1,arg%u(p+1)%p,1)
  end do
  
  do i=l,m-1; deallocate(u(i)%p); enddo
  do i=l+1,m; deallocate(v(i)%p); enddo
